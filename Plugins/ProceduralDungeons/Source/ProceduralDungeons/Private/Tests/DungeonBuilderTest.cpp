...
{
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---TestCorrectDegrees---"));
	int32 TotalTests = 13;
	...
	if (!Builder)
	{
		if (LogLevel >= 1) UE_LOG(LogTemp, Error, TEXT("Builder object was empty. Exiting"));
		return;
	}
	...
	float TestValue = 0.f;
	{
		...
		{
			if (LogLevel >= 1) UE_LOG(LogTemp, Error, TEXT("TestCorrectDegrees: %f failed"), TestValue);
			PassingTests--;
		}
	...
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("%i / %i Cases Pass"), PassingTests, TotalTests);
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---End TestCorrectDegrees---"));
}

void UDungeonBuilderTest::TestGetNextXY()
{
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---TestGetNextXY---"));
	int32 TotalTests = 25;
	int32 PassingTests = TotalTests;
	...
	int32 ExpectedY = -1;
	{
		...
		{
			if (LogLevel >= 1) UE_LOG(LogTemp, Error, TEXT("%f & %f: Expected: %i, %i | Actual: %i, %i"), OpeningOrientation.Yaw, RoomRotation.Yaw, ExpectedX, ExpectedY, X, Y);
			PassingTests--;
		}
		...
		if (X != ExpectedX || Y != ExpectedY)
		{
			if (LogLevel >= 1) UE_LOG(LogTemp, Error, TEXT("%f & %f: Expected: 0, -1 | Actual: %i, %i"), OpeningOrientation.Yaw, RoomRotation.Yaw, ExpectedX, ExpectedY, X, Y);
			PassingTests--;
		}
	...
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("%i / %i Cases Pass"), PassingTests, TotalTests);
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---End TestGetNextXY---"));
}

void UDungeonBuilderTest::TestGetSpawnRotation()
{
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---TestGetSpawnRotation---"));
	int32 TotalTests = 16;
	int32 PassingTests = TotalTests;
	...
	FRotator Actual = Builder->GetSpawnRotation(FirstRotation, SecondRotation, RoomRotation);
	{
		...
		{
			if (LogLevel >= 1) UE_LOG(LogTemp, Error, TEXT("%f, %f & %f: Expected: %f | Actual: %f"), FirstRotation.Yaw, SecondRotation.Yaw, RoomRotation.Yaw, Expected.Yaw, Actual.Yaw);
			PassingTests--;
		}
	...
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("%i / %i Cases Pass"), PassingTests, TotalTests);
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---End TestGetSpawnRotation---"));
}

void UDungeonBuilderTest::TestGetPlacementOffset()
{
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---TestGetPlacementOffset---"));
	int32 TotalTests = 8;
	int32 PassingTests = TotalTests;
	...
	FVector Actual = Builder->GetPlacementOffset(Rotation, Location);
	{
		if (Expected != Actual)
		{
			if (LogLevel >= 1) UE_LOG(LogTemp, Error, TEXT("%f & (%f, %f): Expected: %f,%f | Actual: %f,%f"), Rotation.Yaw, Location.X, Location.Y, Expected.X, Expected.Y, Actual.X, Actual.Y);
			PassingTests--;
		}
	...
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("%i / %i Cases Pass"), PassingTests, TotalTests);
	if (LogLevel >= 3) UE_LOG(LogTemp, Display, TEXT("---End TestGetPlacementOffset---"));
}
...
void UDungeonBuilderTest::NewTestDungeon(FName RoomToSpawn, int32 Socket, float Yaw, FVector StartingLocation)
{
	...
	{
		if (LogLevel >=2) UE_LOG(LogTemp, Warning, TEXT("RoomOpenings2.Num() is empty!"));
		return;
	}
	...
	if (LogLevel >= 4) UE_LOG(LogTemp, Display, TEXT("--- Socket: %i, Yaw: %f ---"), Socket, Yaw);
	if (LogLevel >= 4) UE_LOG(LogTemp, Display, TEXT("Room1 Orientation: [%f, %f, %f]"), SpawnedRoom->GetActorRotation().Pitch, SpawnedRoom->GetActorRotation().Yaw, SpawnedRoom->GetActorRotation().Roll);
	...
	if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Rotation: %f + %f = %f"), RoomOpenings[Socket]->Orientation.Yaw, RoomOpenings2[0]->Orientation.Yaw, RotationToSpawn.Yaw);

	if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Opening1 Location: [%f, %f, %f]"), (RoomLocation + SocketOffset).X, (RoomLocation + SocketOffset).Y, (RoomLocation + SocketOffset).Z);
	FVector LocationOffset = Builder->GetPlacementOffset(RotationToSpawn, RoomOpenings2[0]->Location);
	if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("LocationOffset: [%f, %f, %f]"), LocationOffset.X, LocationOffset.Y, LocationOffset.Z);

	FVector LocationToSpawn = RoomLocation + SocketOffset + LocationOffset;
	if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Final Location: [%f, %f, %f]"), LocationToSpawn.X, LocationToSpawn.Y, LocationToSpawn.Z);
	...
	{
		FDungeonItem* Item = Builder->DungeonData->GetItem(FName(ItemName));
		...
		Rotation.Yaw = Builder->CorrectDegrees(Rotation.Yaw);
		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Room Rotation: [%f, %f, %f]"), RotationToSpawn.Pitch, RotationToSpawn.Yaw, RotationToSpawn.Roll);
		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Item Rotation: [%f, %f, %f]"), Item->Rotation.Pitch, Item->Rotation.Yaw, Item->Rotation.Roll);
		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Rotation: %f + %f = %f"), RotationToSpawn.Yaw, Item->Rotation.Yaw, Rotation.Yaw);

		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Initial Item Location X,Y: [%f, %f]"), Item->Location.X, Item->Location.Y);
		FVector ItemLocation = Builder->GetPlacementOffset(RotationToSpawn, Item->Location);
		FVector Location = LocationToSpawn + ItemLocation;
		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Room Location: [%f, %f, %f]"), LocationToSpawn.X, LocationToSpawn.Y, LocationToSpawn.Z);
		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Item Location: [%f, %f, %f]"), ItemLocation.X, ItemLocation.Y, ItemLocation.Z);
		if (LogLevel >= 4) UE_LOG(LogTemp, Warning, TEXT("Location: %f + %f = %f"), LocationToSpawn.Y, ItemLocation.Y, Location.Y);

		Builder->ItemsHandler->HandleNewItem(Item->Key, Location, Rotation);
	}
}
